...
- Node.js 18 LTS, TypeScript 5.x (backend, frontend) + Express (backend API), Next.js 14 (frontend), React, Tailwind CSS, ExcelJS (Excel parsing), Papa Parse (CSV), Multer (uploads), pg (PostgreSQL client), BullMQ (job queue) (003-db-access-ux)
- PostgreSQL 14+ with per-user schema isolation (`user_<uuid>` pattern) (003-db-access-ux)
...
## Recent Changes
- 003-db-access-ux: Added Node.js 18 LTS, TypeScript 5.x (backend, frontend) + Express (backend API), Next.js 14 (frontend), React, Tailwind CSS, ExcelJS (Excel parsing), Papa Parse (CSV), Multer (uploads), pg (PostgreSQL client), BullMQ (job queue)
