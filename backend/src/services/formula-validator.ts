...
type TokenType = 'NUMBER' | 'OPERATOR' | 'LPAREN' | 'RPAREN' | 'AGGREGATION' | 'IDENTIFIER' | 'EOF';
...
class FormulaValidator {
  ...
  validate(expression: string): void {
    ...
    try {
      ...
      throw new Error(`Invalid formula: ${(err as Error).message}`);
    }
  }
  ...
  private checkDivideByZero(expression: string): void {
